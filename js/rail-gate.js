(function(){
 ...
 var tip=document.createElement('span');tip.className='peek-rail__tip';tip.textContent='按住向左拖动 ←';rail.appendChild(tip);
 ...
 rail.addEventListener('pointerdown',function(e){if(committing||e.button!==0||e.isPrimary===false)return;pressed={id:e.pointerId,x:e.clientX,y:e.clientY,dx:0,dy:0,width:rail.getBoundingClientRect().width,threshold:Math.max(150,Math.min(320,innerWidth*.24))};rail.setPointerCapture(e.pointerId);});
 rail.addEventListener('pointermove',function(e){if(!pressed||e.pointerId!==pressed.id)return;pressed.dx=Math.max(0,pressed.x-e.clientX);pressed.dy=Math.abs(e.clientY-pressed.y);if(pressed.dy>24&&pressed.dy>pressed.dx){reset();return;}if(pressed.dx<12)return;rail.classList.add('rail-dragging');var p=Math.min(.95,pressed.dx/pressed.threshold*.85);rail.style.setProperty('--drag-width',Math.min(innerWidth,pressed.width+pressed.dx)+'px');progress.textContent=Math.round(p*100)+'%';wash.style.opacity=p;});
